...
sealed class Length {
	...
	data class VMIN(val v: Double) : Fixed() {
		override fun calc(ctx: Context): Int = (v * Math.min(ctx.viewportWidth1pc, ctx.viewportHeight1pc)).toInt()
		override fun toString() = "${v}em"
	}

	data class VMAX(val v: Double) : Fixed() {
		override fun calc(ctx: Context): Int = (v * Math.max(ctx.viewportWidth1pc, ctx.viewportHeight1pc)).toInt()
		override fun toString() = "${v}em"
	}
